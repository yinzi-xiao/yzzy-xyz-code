...
error_probability_min, error_probability_max = 0, 1
error_probabilities = np.linspace(error_probability_min, error_probability_max, 25)
...
if __name__ == '__main__':
    ...
    codes = [RotatedPlanarYZCode(d) for d in {77}] # large code size d
    decoder = RotatedPlanarYZRNIIDMPSDecoder(chi=12)
    error_model = BiasedDepolarizingErrorModel(bias=0.5,axis='X')
    ...
    function.save_simdata(os.path.join(save_path,'yzzy_x0.5bias{}.json'.format(task_id)),data)
